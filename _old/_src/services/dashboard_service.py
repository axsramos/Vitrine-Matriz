...
class DashboardService:
    def get_impact_distribution(self, start_date=None, end_date=None):
        """
        Retorna a distribuição de tarefas por Impacto (TrfImp).
        """
        where_clause = None
        params = ()
        
        if start_date and end_date:
            where_clause = "TrfAudIns BETWEEN ? AND ?"
            params = (str(start_date), str(end_date))

        # Busca dados via Model (Tabela T_Trf)
        data = TaskModel.read_all(fields=['TrfImp', 'TrfCod'], where=where_clause, params=params)
        ...
        # Agrupa e renomeia
        df_grouped = df.groupby('TrfImp').size().reset_index(name='Total')
        df_grouped.rename(columns={'TrfImp': 'Impacto'}, inplace=True)
    ...
    def get_tasks_evolution(self, days=30):
        ...
        df['Data'] = pd.to_datetime(df['TrfAudIns']).dt.date
